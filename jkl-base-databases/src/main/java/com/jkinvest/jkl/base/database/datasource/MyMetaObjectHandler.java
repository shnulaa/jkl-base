package com.jkinvest.jkl.base.database.datasource;
 ...
 * <p>
 * 判断逻辑： 1. insert 方法，自动填充 id, createTime, updateTime, createUser, updateUser
 * 字段，字段为空则自动生成，不为空则使用传递进来的 2. update 方法，自动填充 id, updateTime, updateUser 字段，字段为空则自动生成，不为空则使用传递进来的
 * <p>
 * 注入值： id： IdUtil.getSnowflake(workerId, dataCenterId); createTime：LocalDateTime.now()
 * updateTime：LocalDateTime.now() createUser：BaseContextHandler.getUserId()
...
public class MyMetaObjectHandler implements MetaObjectHandler {
   ...
   * 注意：不支持 复合主键 自动注入！！
   * <p>
   * 所有的继承了Entity、SuperEntity的实体，在insert时， id： id为空时， 通过IdGenerate生成唯一ID， 不为空则使用传递进来的id createUser,
   * updateUser: 自动赋予 当前线程上的登录人id createTime, updateTime: 自动赋予 服务器的当前时间
   * <p>
  ...
  public void insertFill(MetaObject metaObject) {
    ...
    if (metaObject.getOriginalObject() instanceof SuperEntity) {
      ...
      if (entity.getCreateUser() == null || entity.getCreateUser().equals(0)) {
        Object userIdVal =
            ID_TYPE.equals(metaObject.getGetterType(SuperEntity.CREATE_USER).getName())
    ...
    if (metaObject.hasGetter(SuperEntity.FIELD_ID)) {
      Object idVal = ID_TYPE.equals(metaObject.getGetterType(SuperEntity.FIELD_ID).getName())
          ? String.valueOf(id)
          : id;
    ...
    TableInfo tableInfo = metaObject.hasGetter(Constants.MP_OPTLOCK_ET_ORIGINAL)
        ? TableInfoHelper
            .getTableInfo(metaObject.getValue(Constants.MP_OPTLOCK_ET_ORIGINAL).getClass())
  ...
  private void update(MetaObject metaObject, Entity entity, String et) {
    if (entity.getUpdateUser() == null || entity.getUpdateUser().equals(0)) {
      Object userIdVal = ID_TYPE.equals(metaObject.getGetterType(et + Entity.UPDATE_USER).getName())
          ? String.valueOf(BaseContextHandler.getUserId())
          : BaseContextHandler.getUserId();
